...
async def test_mdoc_pki_trust_chain(acapy_verifier_admin):
    ...
    requested_attributes = {
        "org.iso.18013.5.1": {"given_name": True}
    }
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            try:
                mdoc_bytes = base64.urlsafe_b64decode(mdoc_cbor_hex + '=' * (-len(mdoc_cbor_hex) % 4))
            except Exception as e:
        ...
        device_authentication_bytes = cbor2.dumps(cbor2.CBORTag(24, cbor2.dumps(device_authentication)))
    ...
    for _ in range(10):
        ...
        pytest.fail(f"Presentation not verified. Final state: {record['state']}, Error: {record.get('error_msg')}")
