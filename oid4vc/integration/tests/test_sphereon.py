...
@pytest.mark.asyncio
async def test_sphereon_accept_mdoc_credential_offer(acapy_issuer_admin, sphereon_client):
...
@pytest.mark.skipif(not MDOC_AVAILABLE, reason="isomdl_uniffi not available")
@pytest.mark.asyncio
async def test_sphereon_present_mdoc_credential(acapy_verifier_admin, acapy_issuer_admin, sphereon_client):
    ...
    offer_response = await acapy_issuer_admin.get("/oid4vci/credential-offer", params={"exchange_id": exchange["exchange_id"]})
    credential_offer = offer_response["credential_offer"]
    
    issue_response = await sphereon_client.post("/oid4vci/accept-offer", json={"offer": credential_offer})
    assert issue_response.status_code == 200
    credential_jwt = issue_response.json()["credential"]
    
    # 2. Create Presentation Request (ACA-Py Verifier)
    # Create verifier DID
    verifier_did_result = await acapy_verifier_admin.post("/did/jwk/create", json={"key_type": "p256"})
...
@pytest.mark.asyncio
async def test_sphereon_accept_credential_offer_by_ref(acapy_issuer_admin, sphereon_client):
