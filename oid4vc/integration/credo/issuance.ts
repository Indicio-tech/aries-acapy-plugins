...
router.post('/accept-offer', async (req: any, res: any) => {
  ...
  try {
    ...
    //
    // Credo 0.6.x rejects JWK binding for W3C credential formats (jwt_vc_json,
    // jwt_vc_json-ld, ldp_vc) — it throws:
    //   "Using a JWK for credential binding is not supported for the '...' format."
    // For those formats we create a did:key and return DID binding instead.
    //
    // For non-W3C formats (mso_mdoc, vc+sd-jwt, dc+sd-jwt) we continue to use
    // JWK binding: the proof JWT carries the holder's public key directly in the
    // 'jwk' header, which ACA-Py verifies via Key.from_jwk() without DID lookup.
    //
    ...
    const credentialBindingResolver = async (bindingOptions: any) => {
        ...
        try {
            ...
            // Use did:key instead — ACA-Py's jwt_vc_json processor handles both
