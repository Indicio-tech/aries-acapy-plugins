...
async function issue_jwt_credential(req, res) {
  ...
  const createCredentialSupportedUrl = `${API_BASE_URL}/oid4vci/credential-supported/jwt/create`;
...
async function issue_sdjwt_credential(req, res) {
  ...
  // Create credential schema
  const createCredentialSupportedUrl = `${API_BASE_URL}/oid4vci/credential-supported/sd-jwt/create`;
  const createCredentialSupportedOptions = {
    method: "POST",
    headers: commonHeaders,
    body: JSON.stringify({
