...
class MsoMdocCredProcessor(Issuer, CredVerifier, PresVerifier):
    ...
    async def _assign_status_entry(
        ...
        if not isinstance(status_handler, StatusHandler):
            LOGGER.info(
    ...
    async def issue(
        ...
        try:
            ...
            # Allocate status only after all issuance inputs have been validated,
            # avoiding orphaned status-list entries when signing cannot proceed.
