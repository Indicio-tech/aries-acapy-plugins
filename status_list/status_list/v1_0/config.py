...
class Config:
    ...
    def from_settings(cls, settings: BaseSettings) -> "Config":
        ...
        list_size = int(
            plugin_settings.get("list_size") or getenv("STATUS_LIST_SIZE") or DEFAULT_LIST_SIZE
        )
        shard_size = int(
            plugin_settings.get("shard_size") or getenv("STATUS_LIST_SHARD_SIZE") or DEFAULT_SHARD_SIZE
        )
